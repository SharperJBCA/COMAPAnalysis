...
if __name__ == "__main__":
    ...
    offsetLength = 20
    noffsets = t.size//offsetLength + 1
    offsets = np.repeat(np.arange(noffsets),offsetLength)[:t.size]
    
    d1 = DataClass()
    
    sigma = np.std(t[1:t.size//2*2:2] - t[:t.size//2*2:2])/np.sqrt(2)
    noiseParams = [sigma, 1., 1, offsetLength/25.]
    sr = 25.
    d1.setData(t, pixels, offsets, npix, noffsets, offsetLength/sr, noiseParams)
    CG(d1)
    d1.EstimatePower()
    
    m = d1.ReturnMap()
    #sky = d1.ReturnMap(nonoise)

    ds = d1.ReturnDestripedMap()
    
    ps = np.abs(np.fft.fft(d1.a0))**2
    f = np.fft.fftfreq(ps.size, d=offsetLength/25.)
    pyplot.plot(ftod[1:ftod.size//2], pstod[1:ftod.size//2]/ftod.size,'-')
    pyplot.plot(f[1:f.size//2], ps[1:f.size//2]/d1.a0.size,'-')
    pyplot.show()
    
    msky = FileTools.ReadH5Py('Continuous_El37_4-2deg_Full_25Hz_1Channel_60Day_ALL_sim_SkyTOD_NoNoise.hdf5')['MAPS'].flatten()
    msky = hp.ud_grade(msky, nside)
    #hp.write_map()
    hp.mollview(ds,norm='hist')
    hp.mollview(m-ds-msky,norm='hist')
    hp.mollview(m-msky,norm='hist')
    pyplot.show()
    #print(d2.xVec , d2.a0)
    #stop
    
    pyplot.plot(d1.tod)
    pyplot.plot(np.repeat(d1.a0, offsetLength)[:d1.tod.size])
    pyplot.show()
